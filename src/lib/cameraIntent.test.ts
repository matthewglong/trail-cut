...
import { circularLerp } from './routeLocation';
import {
...
/** Build an IndexedRoute stub with just `points` + `minTimeMs`/`maxTimeMs`.
 *  The cumulative distance/mercator fields are zero-filled — these tests
 *  exercise time-based lookups (locationAt, bearingFromKeyframes) that don't
 *  read those parallel arrays. */
function stubIndexedRoute(
  points: import('./routeLocation').IndexedPoint[],
): import('./routeLocation').IndexedRoute {
  const zeros = new Array<number>(points.length).fill(0);
  return {
    points,
    minTimeMs: points.length ? points[0].timeMs : 0,
    maxTimeMs: points.length ? points[points.length - 1].timeMs : 0,
...
// downstream consumers (the transition-span evaluator) rely on.
// ---------------------------------------------------------------------------
...
describe('cameraAt(timeline, t)', () => {
  ...
  it('inside a clip span (follow intent): translates project-time → wall-clock per the plan', () => {
    // Follow-mode settings + a route so anchorIntentForClip emits a follow
    // intent for the test clip.
    const t0 = Date.parse('2026-04-04T15:00:00Z');
    const route = stubIndexedRoute([
      { lat: 37.77, lng: -122.40, timeMs: t0 },
