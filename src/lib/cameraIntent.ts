...
export interface TransitionSpan {
  ...
  /** Whether this span drives the CAMERA. `false` only for intra-group
   *  seams (both `fromClipId` and `toClipId` in the same compile-effective
   *  clip group): the span still exists — same placement and duration as
   *  ungrouped — so the marker layer (travel playhead, seam eases,
   *  `activeClipIdAt`) keeps firing exactly as configured, but the camera
   *  stays on the group's glide across the cut. Baked at compile; the
   *  evaluator never arbitrates between authorities at runtime
   *  (`docs/CLIP_GROUPS_HANDOFF.md` §2 "Dispatch"). Default `true`. */
  cameraAuthority: boolean;
}

/** One camera stop generated by a clip group's sweep rule. Stores a
 *  `ResolvedCamera` directly: the sweep rule only ever produces point-like
 *  cameras (center from `locationAt`, scalar zoom/bearing/pitch) — no
 *  `region` intent is involved, so nothing here is viewport-dependent.
 *  Zoom stays reference-space (`docs/CANON.md` §2.6). */
export interface GroupAnchor {
  /** Project-time, ms; strictly increasing within a `GroupSpan`. */
  tMs: number;
  /** Camera snapshot at `tMs`. */
  camera: ResolvedCamera;
}

/** A clip group's contribution to the compiled timeline: one continuous
 *  camera glide spanning its member clips' spans end-to-end. Inside
 *  `[startMs, endMs]` the group outranks the member clip spans in
 *  `cameraAt` dispatch (members' `follow_playhead` is ignored while
 *  grouped). See `docs/CLIP_GROUPS_HANDOFF.md` §"Decided model". */
export interface GroupSpan {
  groupId: string;
  /** Compile-effective members, timeline order. */
  memberClipIds: string[];
  /** == first member `ClipSpan.startMs`. */
  startMs: number;
  /** == last member `ClipSpan.endMs`. */
  endMs: number;
  /** Length == member count, ≥ 2. Anchor 1 sits at `startMs`, anchor n at
   *  `endMs`; every intermediate anchor lives inside its member's span. */
  anchors: GroupAnchor[];
}
...
export interface CompiledTimeline {
  ...
  /** Camera-glide spans from clip groups, timeline order, pairwise
   *  disjoint. Empty for an ungrouped project. */
...
/** Inverse of `lngLatToMercator`: world-pixel coordinates at zoom 0 back
 *  to `LngLat`. Round-trips exactly (to float precision) inside the
 *  Mercator-valid band. Used by the clip-group glide, which splines
 *  through mercator space so a constant-velocity glide is straight on the
 *  map rather than in degrees. */
function mercatorToLngLat(p: { x: number; y: number }): LngLat {
  const lng = (p.x / WORLD_SIZE_AT_ZOOM_0) * 360 - 180;
  const n = Math.PI * (1 - (2 * p.y) / WORLD_SIZE_AT_ZOOM_0);
  const lat = (Math.atan(Math.sinh(n)) * 180) / Math.PI;
  return { lng, lat };
}

/** Pitch (degrees) implied by a map style: '3d' tilts to 60, every other
 *  style is top-down. The one place this mapping lives. */
function pitchForMapStyle(mapStyle: MapSettings['camera']['map_style']): number {
  return mapStyle === '3d' ? 60 : 0;
}
...
function anchorIntentForClip(
  ...
  const pitch = pitchForMapStyle(settings.camera.map_style);
...
export interface CompileTimelineProjectSettings {
  ...
  /** Clip groups (camera-glide generators). Absent ⇔ empty. The compiler
   *  re-normalizes against its filtered clip list via `normalizeClipGroups`
   *  (`src/lib/clipGroups.ts`). */
...
export function resolveProjectStartCamera(
  ...
  return {
    ...
    zoom: 12,
    bearing: 0,
    pitch: pitchForMapStyle(projectMapSettings.camera.map_style),
  };
}
...
export function compileTimeline(
  ...
  // Per-clip fully-resolved MapSettings, stashed for `buildGroupSpans` so
  // the group anchors read the exact same resolution the clip intent did.
  const resolvedSettingsByClipId = new Map<string, MapSettings>();
  let cursor = 0;
  for (const clip of validClips) {
    ...
    resolvedSettingsByClipId.set(clip.id, settingsForClip);
  ...
  // Clip groups → camera-glide spans. Built after clip spans (anchors are
  // placed on the project-time axis the spans define) and before
  // transition spans (entry/exit seams take their endpoint snapshots from
  // the glide, and intra-group seams lose camera authority).
  const groupSpans = buildGroupSpans(
    validClips,
    clipSpans,
    resolvedSettingsByClipId,
    route,
    settings.clip_groups ?? [],
  );
  const groupIdByClipId = new Map<string, string>();
  const groupByFirstMember = new Map<string, GroupSpan>();
  const groupByLastMember = new Map<string, GroupSpan>();
  for (const g of groupSpans) {
    for (const id of g.memberClipIds) groupIdByClipId.set(id, g.groupId);
    groupByFirstMember.set(g.memberClipIds[0], g);
    groupByLastMember.set(g.memberClipIds[g.memberClipIds.length - 1], g);
  }
  ...
  for (let i = 0; i < clipSpans.length; i++) {
    ...
    // Intra-group seam: both sides in the same compile-effective group.
    // The span is still emitted exactly as ungrouped (placement, authored
    // and auto durations) so the marker layer's travel windows / seam
    // eases are unchanged by grouping; only the camera claim is removed.
    const prevGroupId = prevClip ? groupIdByClipId.get(prevClip.id) : undefined;
    const currGroupId = groupIdByClipId.get(currClip.id);
    const cameraAuthority = !(
      prevGroupId !== undefined && prevGroupId === currGroupId
    );
    // Entry/exit seams: the arc endpoint on the grouped side is the glide
    // camera at the CUT (anchor 1 / anchor n) for the auto-duration
    // snapshot — deterministic without knowing the window width, matching
    // the existing "duration from clip-edge snapshot, evaluation from
    // window-edge" pattern (`evaluateTransitionSpan` samples the glide at
    // the window edge instead). Adjacent groups compose: both apply.
    const exitGroup =
      cameraAuthority && prevClip ? groupByLastMember.get(prevClip.id) : undefined;
    const entryGroup = cameraAuthority
      ? groupByFirstMember.get(currClip.id)
      : undefined;
    ...
    if (authored.enabled === false) {
      ...
      if (authored.duration_ms != null) {
        ...
        const fromCamera = exitGroup
          ? exitGroup.anchors[exitGroup.anchors.length - 1].camera
          : prevSpan && prevClip
            ? resolveCanonical(
                prevSpan.intent,
            ...
            : startCamera;
        const toCamera = entryGroup
          ? entryGroup.anchors[0].camera
          : resolveCanonical(
              currSpan.intent,
  ...
  return {
    ...
    transitionSpans,
    groupSpans,
    // Grouping never adds frames: the group spans tile the same clip spans.
...
// -- Clip groups → camera-glide spans ---------------------------------------

/** Generate the camera stops for every compile-effective clip group
 *  (`docs/CLIP_GROUPS_HANDOFF.md` §"Decided model", §2 `buildGroupSpans`).
 *
 *  Sweep rule: member k (1-based) of n contributes one anchor at fraction
 *  `f = (k−1)/(n−1)` through ITS OWN trimmed span — clip 1 → its first
 *  frame, middle → midpoint, clip n → its last frame. Anchor 1/n coincide
 *  with the group's first/last frames; every intermediate anchor lives
 *  inside a clip, so every member cut lands mid-glide.
 *
 *  Anchor center is TIME-sampled at the hiker's position for that playback
 *  moment (`wallMs` uses the identical project-time → clip-local → wall-
 *  clock translation as `liveIntentForClipSpan`, so trim and speed are
 *  honored). Zoom / bearing / pitch come from the member's fully resolved
 *  MapSettings (per-clip overrides shape the glide).
 *
 *  Auto bearing samples `bearingAt` directly at the anchor wall-clock —
 *  NOT the per-clip stop-quantized `bearingKeyframes` the ungrouped follow
 *  camera reads. The slight difference from ungrouped follow bearing is by
 *  design (grouping replaces camera authority; handoff §8.5); the
 *  `?? previous anchor ?? 0` chain covers route edges where `bearingAt`
 *  has no window.
 *
 *  A group whose anchors contain any non-finite number is DROPPED (its
 *  members degrade to per-clip camera behavior) rather than compiled: a
 *  NaN anchor would poison every spline sample across the whole glide,
 *  and rendering NaN is worse than losing the glide for that group. */
function buildGroupSpans(
  validClips: Clip[],
  clipSpans: ClipSpan[],
  resolvedSettingsByClipId: Map<string, MapSettings>,
  route: IndexedRoute | null,
  authoredGroups: ClipGroup[],
): GroupSpan[] {
  if (authoredGroups.length === 0) return [];

  const groups = normalizeClipGroups(authoredGroups, validClips);
  if (groups.length === 0) return [];

  const clipById = new Map<string, Clip>();
  for (const c of validClips) clipById.set(c.id, c);
  const spanByClipId = new Map<string, ClipSpan>();
  for (const s of clipSpans) spanByClipId.set(s.clipId, s);

  const out: GroupSpan[] = [];
  for (const group of groups) {
    const n = group.clip_ids.length;
    // normalizeClipGroups guarantees n ≥ 2 and every id present; guard
    // anyway so a policy change upstream can't produce a 0-length glide.
    if (n < 2) continue;
    ...
    let finite = true;
    for (let k = 0; k < n; k++) {
      const clipId = group.clip_ids[k];
      const clip = clipById.get(clipId);
      const span = spanByClipId.get(clipId);
      const resolved = resolvedSettingsByClipId.get(clipId);
      if (!clip || !span || !resolved) {
        finite = false;
        break;
      }

      const f = k / (n - 1);
      const tMs = span.startMs + f * (span.endMs - span.startMs);
      const wallMs =
        span.wallClockBaseMs +
        span.mediaInMs +
        f * (span.mediaOutMs - span.mediaInMs);

      const loc = locationAt(wallMs, route, clip.gps);
      const center: LngLat = loc
        ? { lng: loc.lng, lat: loc.lat }
        : { lng: 0, lat: 0 };

      let bearing: number;
      if (resolved.camera.bearing_mode === 'auto') {
        bearing = bearingAt(wallMs, route) ?? prevBearing ?? 0;
      } else {
        bearing = resolved.camera.bearing_degrees;
      }
      prevBearing = bearing;
      ...
      const camera: ResolvedCamera = {
        ...
        bearing,
        pitch: pitchForMapStyle(resolved.camera.map_style),
      };
      ...
      anchors.push({ tMs, camera });
    }
    if (!finite) continue;

    out.push({
      groupId: group.id,
      memberClipIds: [...group.clip_ids],
      startMs: anchors[0].tMs,
      endMs: anchors[anchors.length - 1].tMs,
      anchors,
    });
  }
  return out;
...
/** Evaluate a group's glide at project-time `t` (clamped into
 *  `[span.startMs, span.endMs]`). Pure; the splines are rebuilt per call —
 *  n is tiny and the timeline crosses IPC as plain data, so no cache.
 *
 *  - center: centripetal Catmull-Rom Hermite through the anchors' mercator
 *    points on the time knots, back-projected via `mercatorToLngLat`.
 *  - zoom / pitch: Fritsch–Carlson monotone Hermite (change-then-hold
 *    cannot bounce).
 *  - bearing: cumulative short-way unwrap → monotone Hermite → wrap to
 *    [0, 360). The monotone builder is used here too so a heading that
 *    swings then holds cannot overshoot past its final value.
 *  - No easing: u is linear in t per segment (handoff §2). */
export function groupCameraAt(span: GroupSpan, t: number): ResolvedCamera {
  const { anchors } = span;
  const tc = t < span.startMs ? span.startMs : t > span.endMs ? span.endMs : t;
  const knots = anchors.map((a) => a.tMs);

  const merc: Vec2[] = anchors.map((a) => lngLatToMercator(a.camera.center));
  const centerSpline = buildCentripetalSpline2D(knots, merc);
  const center = mercatorToLngLat(evalSpline2D(centerSpline, tc));

  const zoom = evalSpline1D(
    buildMonotoneSpline1D(knots, anchors.map((a) => a.camera.zoom)),
    tc,
  );
  const pitch = evalSpline1D(
    buildMonotoneSpline1D(knots, anchors.map((a) => a.camera.pitch)),
    tc,
  );
  const bearing = wrapDegrees(
    evalSpline1D(
      buildMonotoneSpline1D(
        knots,
        unwrapDegrees(anchors.map((a) => a.camera.bearing)),
      ),
      tc,
    ),
  );
...
/** The group span whose FIRST member is `clipId`, if any — the group a
 *  transition with `toClipId === clipId` enters. */
export function groupSpanWithFirstMember(
  timeline: CompiledTimeline,
  clipId: string,
): GroupSpan | null {
  for (const g of timeline.groupSpans) {
    if (g.memberClipIds[0] === clipId) return g;
  }
  return null;
}

/** The group span whose LAST member is `clipId`, if any — the group a
 *  transition with `fromClipId === clipId` exits. */
export function groupSpanWithLastMember(
  timeline: CompiledTimeline,
  clipId: string,
): GroupSpan | null {
  for (const g of timeline.groupSpans) {
    if (g.memberClipIds[g.memberClipIds.length - 1] === clipId) return g;
  }
  return null;
}
...
/** `findTransitionSpanAt` restricted to CAMERA-authoritative spans. Same
 *  closed-interval, zero-duration-skip and later-wins semantics; spans with
 *  `cameraAuthority === false` (intra-group seams) are invisible here, so
 *  the camera stays on the group glide across a member cut while the
 *  marker layer (which uses the unfiltered lookup) still sees the seam. */
export function findCameraTransitionSpanAt(
  spans: TransitionSpan[],
  t: number,
): TransitionSpan | null {
  let result: TransitionSpan | null = null;
  for (const s of spans) {
    if (!s.cameraAuthority) continue;
    if (s.effectiveDurationMs <= 0) continue;
    if (t >= s.startMs && t <= s.endMs) result = s;
  }
  return result;
...
/** Find the group span containing project-time `t`. Half-open
 *  `[start, end)` like clip spans, except the group that ends the timeline
 *  (`endMs >= totalDurationMs`) is closed on the right so
 *  `t == totalDurationMs` lands on the glide's final frame. */
function findGroupSpanAt(
  spans: GroupSpan[],
  t: number,
  totalDurationMs: number,
): GroupSpan | null {
  for (const g of spans) {
    const closesTimeline = g.endMs >= totalDurationMs;
    if (t >= g.startMs && (closesTimeline ? t <= g.endMs : t < g.endMs)) {
      return g;
    }
  }
  return null;
...
/** Promote a glide sample to a `point` CameraIntent. */
function groupCameraAsPointIntent(span: GroupSpan, t: number): CameraIntent {
  const cam = groupCameraAt(span, t);
  return {
    kind: 'point',
    center: cam.center,
    zoom: cam.zoom,
...
function evaluateTransitionSpan(
  ...
  //
  // Group entry/exit: when the destination clip is a group's FIRST member
  // (or the source its LAST), that endpoint is the glide camera at the
  // window edge — `groupCameraAt(group, span.endMs)` / `(group,
  // span.startMs)` — so the arc lands exactly where the glide is when the
  // group takes over at `span.endMs` (mid-glide inside clip 1, since the
  // post-cut half of the window extends into the group), and symmetric on
  // exit. Both rules compose on the seam between two adjacent groups.
  const exitGroup = span.fromClipId
    ? groupSpanWithLastMember(timeline, span.fromClipId)
    : null;
  const entryGroup = groupSpanWithFirstMember(timeline, span.toClipId);

  const fromCamera: ResolvedCamera = exitGroup
    ? groupCameraAt(exitGroup, span.startMs)
    : fromSpan
      ? resolveCanonical(
          fromSpan.intent,
      ...
      : startCamera;
  const toCamera: ResolvedCamera = entryGroup
    ? groupCameraAt(entryGroup, span.endMs)
    : resolveCanonical(
        toSpan.intent,
...
export function cameraAt(timeline: CompiledTimeline, t: number): CameraIntent {
  const { clipSpans, transitionSpans, groupSpans, totalDurationMs, startCamera } =
    timeline;
  ...
  if (t >= totalDurationMs) {
    ...
    // A grouped final clip holds the glide's terminal frame.
    const lastGroup = groupSpanWithLastMember(timeline, last.clipId);
    if (lastGroup) return groupCameraAsPointIntent(lastGroup, lastGroup.endMs);
    return liveIntentForClipSpan(last, last.endMs);
  }

  // Disjoint compile-time authority, in order: camera-authoritative
  // transition arcs → group glides → clip spans. Intra-group seams carry
  // `cameraAuthority: false` and are skipped here (the group glide owns the
  // cut); nothing is arbitrated at runtime.
  const transitionSpan = findCameraTransitionSpanAt(transitionSpans, t);
  if (transitionSpan) {
    return evaluateTransitionSpan(timeline, transitionSpan, t);
  }

  const groupSpan = findGroupSpanAt(groupSpans, t, totalDurationMs);
  if (groupSpan) return groupCameraAsPointIntent(groupSpan, t);
